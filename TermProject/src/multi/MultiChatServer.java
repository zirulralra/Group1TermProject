		...
		public void run()  // 스레드에서 처리할 일을 기재한다
		{
			...
			try {
				...
				while(status) {
					...
					else if (m.getType().equals("PROFILE_REQUEST")) {
					    String targetId = m.getRcvid();     // 보고 싶은 대상 (친구)
					    String requesterId = m.getId();     // 요청자

					    User user = UserDatabase.shared().getUserById(targetId);

					    Message response = new Message();
					    response.setType("PROFILE_RESPONSE");
					    response.setId("server");            // 응답 발신자
					    response.setRcvid(targetId);         // ✅ 친구 ID (조회 대상 ID)

					    if (user != null) {
					        response.setProfile("{\"nickname\":\"" + user.getNickname() + "\", \"intro\":\"" + user.getIntro() + "\"}");
					    } else {
					        response.setProfile("{\"nickname\":\"정보 없음\", \"intro\":\"\"}");
					    }

					    for (ChatThread ct : chatThreads) {
					        if (ct.clientId != null && ct.clientId.equals(requesterId)) {
					            ct.outMsg.println(gson.toJson(response));
					            System.out.println("[서버] " + requesterId + "에게 PROFILE_RESPONSE 전송됨 (rcvid: " + targetId + ")");
					            break;
					        }
					    }
					}

					else if (m.getType().equals("PROFILE_SAVE")) {
					    String userId = m.getId();

					    JsonObject profileJson = JsonParser.parseString(m.getProfile()).getAsJsonObject();
					    String nickname = profileJson.get("nickname").getAsString();
					    String intro = profileJson.get("intro").getAsString();

					    // UserProfileManager 저장 (기존)
					    UserProfileManager.saveProfile(userId, nickname, intro);

					    // UserDatabase도 업데이트
					    User user = UserDatabase.shared().getUserById(userId);
					    if (user != null) {
					        user.setNickname(nickname);
					        user.setIntro(intro);
					        System.out.println("[서버] UserDatabase에 닉네임 업데이트 완료: " + nickname);
					    }
					}
