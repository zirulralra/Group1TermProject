...
import Profile.ProfileController; // 프로필 컨트롤러 추가
...
public class FriendListUI extends JFrame {
    ...
    private JButton profileButton; // 프로필 버튼 추가
    private JButton myProfileButton; // 내 프로필 버튼 추가
    ...
    private void initUI() {
        ...
        myProfileButton = new JButton("내 프로필");
        stylizeButton(myProfileButton);
        myProfileButton.setFont(new Font("SansSerif", Font.PLAIN, 12));
        myProfileButton.addActionListener(e -> openMyProfile());
        userInfoPanel.add(myProfileButton);
        ...
        profileButton = new JButton("프로필 보기");
        stylizeButton(profileButton);
        profileButton.addActionListener(e -> viewSelectedFriendProfile());
        ...
        friendButtonPanel.add(profileButton);
    ...
    // 프로필 관련 메소드 추가
    private void viewSelectedFriendProfile() {
        String selectedFriend = friendList.getSelectedValue();
        if (selectedFriend != null) {
            ProfileController.openProfile(selectedFriend);
        } else {
            JOptionPane.showMessageDialog(this, "프로필을 볼 친구를 선택해주세요.");
        }
    }
    
    private void openMyProfile() {
        ProfileController.openProfile(currentUserId);
    }
}